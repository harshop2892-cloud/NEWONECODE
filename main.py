...
active_sessions = {}
semaphore = asyncio.Semaphore(25)  # Fast response ke liye
...
async def check_card_with_retry(card, sites, proxies, max_retries=2):
    ...
    if last_result:
        return {'status': 'Dead', 'message': f'Site errors: {last_result["message"]}', 'card': card, 'gateway': last_result.get('gateway', 'Unknown'), 'price': last_result.get('price', '-')}
...
# ... (baaki functions jaise update_progress, send_final_results, test_site, test_proxy, admin commands etc. same rakh sakte ho)
...
async def check_command(event):
    ...
    if len(cards) > 5000:
        await status_msg.edit(premium_emoji(f"⚠️ Limiting to first 5000 cards."), parse_mode='html')
        cards = cards[:5000]
